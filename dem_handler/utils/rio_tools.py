...
def translate_dataset(
    dataset: DatasetReader, x_shift: float, y_shift: float
...
def reproject_arr_to_new_crs(
    ...
    reprojected_profile = reproject_profile_to_new_crs(
        src_profile, dst_crs, target_resolution=tr
    )
    resampling = Resampling[resampling]
    dst_array = np.zeros(
        (
            reprojected_profile["count"],
...
def update_profile_resolution(
    ...
    dst_transform, dst_width, dst_height = _aligned_target(
